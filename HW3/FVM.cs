...
public class FVM : MonoBehaviour
{
	...
	SVD svd = new SVD();
    [Header("Principle stretches")]
    public bool use_Principle_Stretches = false;
    ...
    void _Update()
    {
        ...
        for (int tet=0; tet<tet_number; tet++)
        {
            ...
            //Green Strain
            if (!use_Principle_Stretches)
            {
                //Green Strain
                Matrix4x4 G = matrixMultiplyFloat(matrixSubmatrix(F.transpose * F, Matrix4x4.identity), 0.5f);
                //First PK Stress
                Matrix4x4 S = matrixAddmatrix(matrixMultiplyFloat(G, 2 * stiffness_1), matrixMultiplyFloat(Matrix4x4.identity, stiffness_0 * trace(G)));
                //Second PK stress is P = F * S
                //Elastic Force
            ...
            else
            {
                ...
                float dWdIIIc = 0;
                float dIcdlamda0 = 2 * D[0, 0];
                float dIcdlamda1 = 2 * D[1, 1];
                float dIcdlamda2 = 2 * D[2, 2];
                float dIIcdlamda0 = 4 * D[0, 0] * D[0, 0] * D[0, 0];
                float dIIcdlamda1 = 4 * D[1, 1] * D[1, 1] * D[1, 1];
                float dIIcdlamda2 = 4 * D[2, 2] * D[2, 2] * D[2, 2];
                float dIIIcdlamda0 = 2 * IIIc / D[0, 0];
                float dIIIcdlamda1 = 2 * IIIc / D[1, 1];
                float dIIIcdlamda2 = 2 * IIIc / D[2, 2];
